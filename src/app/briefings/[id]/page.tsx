...
import { collection, orderBy, query, doc, deleteDoc, getDocs, writeBatch, updateDoc, serverTimestamp } from 'firebase/firestore';
import {
  ColumnDef,
  flexRender,
  getCoreRowModel,
...
import { MoreHorizontal, PlusCircle, ArrowUpDown, Loader2, Trash2, Edit, FileDown, Users, CheckCircle } from 'lucide-react';
...
export default function BriefingsPage() {
  ...
  const { data: briefings, isLoading: isLoadingBriefings, refetch: refetchBriefings } = useCollection<Briefing>(briefingsQuery);
  ...
  const handlePrint = async () => {
    ...
    try {
        const ComponentToPrint = PrintableBriefing({ briefing: selectedBriefing, participants: participants || [] });
        await generatePdfFromComponent(
            ComponentToPrint,
            `Briefing - ${selectedBriefing.area} - ${format(new Date(selectedBriefing.briefingDate), 'yyyy-MM-dd')}.pdf`
        );
    } catch (error) {
   ...
   const handleAcknowledge = async () => {
        ...
        try {
            ...
            toast({
                ...
                description: "Anda telah menyetujui materi briefing ini.",
            });
            refetchBriefings(); // Re-fetch all briefings data
            // Also update the selected briefing in state to reflect the change immediately in the modal
            setSelectedBriefing(prev => prev ? { ...prev, acknowledgedBy: user.uid, acknowledgerName: userProfile.name, acknowledgedAt: new Date() } : null);
        } catch (error) {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => openDetailModal(briefing)}>Lihat Detail & Cetak</DropdownMenuItem>
                ...
                <>
                    ...
                    <div className="pt-4 border-t mt-auto">
                        <Button onClick={handlePrint} disabled={isProcessing}>
                            {isProcessing ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <FileDown className="mr-2 h-4 w-4" />}
                            Download PDF
                        </Button>
                    </div>
                </>
